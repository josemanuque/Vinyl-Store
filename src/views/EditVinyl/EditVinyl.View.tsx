...
import { createVinyl, getVinylById, updateVinyl } from '../../services/VinylService';
import { VinylInputInterface } from '../../interfaces/Vinyl.Interface';
import { createTracks, deleteTracks } from '../../services/TrackService';
...
const EditVinylView: FC = () => {
    ...
    const [oldTracks, setOldTracks] = useState<TrackInterface[]>([]);
    const [tracks, setTracks] = useState<TrackInterface[]>([]);
    const [isTrackListEmpty, setIsTrackListEmpty] = useState<boolean>(true);
    ...
    useEffect(() => {
        ...
        const fetchVinyl = async () => {
            ...
            setReleaseDate(data.releaseDate? data.releaseDate : '');
            setDescription(data.description? data.description : '');
            setTracks(data.tracks);
            setOldTracks(data.tracks);
            if (data.tracks.length > 0) {
                setIsTrackListEmpty(false);
            }
        }
        fetchVinyl();
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try{
            const updatedVinyl = await updateVinyl(id!, generateVinyl());
            const createdTracks = tracks.map((track) => ({
                ...
                albumId: updatedVinyl.id,
                artistId: artist
            }));
            // Call API to save the tracks
            if (!isTrackListEmpty) {
                await deleteTracks(oldTracks.map(track => track.id));
            }
            await createTracks(createdTracks);
            // Redirect to the vinyl details page
            navigate(`/vinyls/${updatedVinyl.id}`);
        } catch (error) {
